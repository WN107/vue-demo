...
module.exports = {
  ...
  module: {
    ...
    rules: [
      ...
      {
        // 这个属性是一个正则表达式，用于匹配文件。这里匹配的是.vue的文件
        test: /\.vue$/,
        // 指定该类型文件的加载器名称
        loader: 'vue-loader',
        // 针对此加载器的具体配置
        // 针对前面的分析，这个配置对象中包含了各种css类型文件的配置，css source map的配置以及一些transform的配置
        options: vueLoaderConfig
      },
      {
        // .js文件的配置
        test: /\.js$/,
        // js文件的处理主要使用的是babel-loader，在这里没有具体的编译规则，
        // babel-loader会自动读取根目录下面的.babelrc中的babel配置用于编译js文件
        /*
          * {
         * // 使用的预设
            "presets": [
