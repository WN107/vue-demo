// dev环境的配置
'use strict'
// 首先引入的是一些工具方法，下面我们就需要去util文件看一下有哪些对应的工具方法
...
const devWebpackConfig = merge(baseWebpackConfig, {
  ...
  devServer: {
    ...
    // 如果两者都希望显示，则把这两项都设置为true
      ...
      // 设置为false则表示都不显示
      : false,
    // 指定webpack-dev-server的根目录，这个目录下所有的文件都是能直接通过浏览器访问的
    // 推荐和output.publicPath设置为一致
    publicPath: config.dev.assetsPublicPath,
    // 配置代理，这样我们就可以跨域访问某些接口
    // 我们访问的接口。如果符合这个选项的配置，就会通过代理服务器转发我们的请求
    proxy: config.dev.proxyTable,
    // 启用quiet后，除了初始启动信息之外的任何内容都不会被打印到控制台。这也意味着来自webpack的错误或警告在控制台看不到
  ...
  plugins: [
    ...
    // 这个插件的只要作用就是在热加载的时候返回更新文件的名称，而不是文件的id
  ...
  portfinder.getPort((err, port) => {
    if (err) {
      ...
      devWebpackConfig.plugins.push(new FriendlyErrorsPlugin({
        ...
        // 当编译出错时，根据congfig.dev.notifyOnError来确定是否需要在桌面右上角显示错误通知框
