...
// 这个模块主要用于在webpack中拷贝文件盒文件夹
const CopyWebpackPlugin = require('copy-webpack-plugin')
// 引入html-webpack-plugin插件
// 这个模块主要用于基于模板生成html文件
...
const webpackConfig = merge(baseWebpackConfig, {
  ...
  output: {
    ...
    // [id]也是一个占位符，表示的是模块标签符(module identifier)
  ...
  plugins: [
    ...
    new UglifyJsPlugin({
      ...
      // 使用source map将错误信息的位置映射到模块(这会减慢编译的速度)
    ...
    new ExtractTextPlugin({
      ...
      // 从所有额外的chunk(additional chunk)提取css内容
      // (默认情况下，它仅从初始chunk(initial chunk)中提取)
      // 当使用 CommonsChunkPlugin并且在公共chunk中提取的chunk(来自ExtractTextPlugin.extract)时
      // 这个选项需要设置为false
      allChunks: true,
    }),
    ...
    // 为了将项目中的第三方依赖代码抽离出来，官方文档上推荐使用这个插件，当我们在项目中实际使用之后，
    // 发现一旦更改了app.js内的代码，vendor.js的hash也会改变，那么下次上线时，
    // 用户仍然需要重新下载 vendor.js与app.js这样就失去了缓存的意义了，所以第二次new就是解决这个问题
